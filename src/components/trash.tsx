...
import { clearJobs, removeJob } from "../redux/jobs-reducer";
...
export function DeleteOneJob({ callBack }: { callBack: () => void }) {
  ...
  const message = (
    <span>
      Are you sure you want to DELETE this job? It will be permanently removed.
    </span>
  );
...
export function DeleteAllJobs({ callBack }: { callBack: () => void }) {
  const dispatch = useAppDispatch();
  const message = (
    <span>
      Are you sure you want to DELETE{" "}
      <span className="text-lg font-bold text-rose-600">ALL</span> jobs? They
      will be permanently removed.
    </span>
  );

  const action = async () => {
    console.log("delete all jobs");
    const jobs = await library.getJobs();
    const imagePaths = jobs.map((job) => appCommon.getImagePath(job.filename));

    library.deleteAllJobs();
    dispatch(clearJobs());
    dispatch(navigate("job-list"));
